...
(function () {
    ...
    var DomModule = Nova({
        ...
        _parseDependencies: function _parseDependencies() {
            ...
            var dependScripts = Array.prototype.slice.call(ele.querySelectorAll('script[src]'));
            ...
            dependScripts.forEach(function (script) {
                // 确保baseUrl后面有斜杠
                var baseUrl = Nova.devOpt.umd.baseUrl;
                if (baseUrl[baseUrl.length - 1] != '/') {
                    baseUrl += '/';
                }

                var relativeSrc = script.getAttribute('src').replace(baseUrl, '').replace(/\.js$/, '');
                var exports = script.getAttribute('exports') || '_' + Math.floor(Math.random() * 1000);
        ...
        _htmlToJs: function _htmlToJs() {
            ...
            var script = ele.querySelector('script:not([src])');
