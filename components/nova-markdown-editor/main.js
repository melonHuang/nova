  ...
  var e;!(function (e) {
    ...
    var e;!(function (e) {
      function t() {
        ...
        switch (typeof e) {case "undefined":
            this.dialect = E.dialects.Gruber;break;case "object":
            this.dialect = e;break;default:
            if (!(e in E.dialects)) throw new Error("Unknown Markdown dialect '" + String(e) + "'");this.dialect = E.dialects[e];}this.em_state = [], this.strong_state = [], this.debug_indent = "";
      };h.dialects = {};var p = h.mk_block = l.mk_block,
          c = l.isArray;h.parse = function (e, t) {
        var n = new h(t);return n.toTree(e);
      }, h.prototype.split_blocks = function (e) {
        ...
        function t() {}function n() {}return (t.prototype = e.block, n.prototype = e.inline, { block: new t(), inline: new n() });
          ...
          g = l.isEmpty,
          y = v.inline_until_char,
          b = { block: { atxHeader: function atxHeader(t, n) {
            var r = t.match(/^(#{1,6})\s*(.*?)\s*#*\s*(?:\n|$)/);if (!r) return void 0;var i = ["header", { level: r[1].length }];return (Array.prototype.push.apply(i, this.processInline(r[2])), r[0].length < t.length && n.unshift(p(t.substr(r[0].length), t.trailing, t.lineNumber + 2)), [i]);
          }, setextHeader: function setextHeader(t, n) {
            var r = t.match(/^(.*)\n([-=])\2\2+(?:\n|$)/);if (!r) return void 0;var i = "=" === r[2] ? 1 : 2,
                s = ["header", { level: i }, r[1]];return (r[0].length < t.length && n.unshift(p(t.substr(r[0].length), t.trailing, t.lineNumber + 2)), [s]);
          }, code: function code(t, n) {
            var r = [],
                i = /^(?: {0,3}\t| {4})(.*)\n?/;if (!t.match(i)) return void 0;e: for (;;) {
              var s = this.loop_re_over_block(i, t.valueOf(), function (e) {
                r.push(e[1]);
              });if (s.length) {
                n.unshift(p(s, t.trailing));break e;
              }if (!n.length) break e;if (!n[0].match(i)) break e;r.push(t.trailing.replace(/[^\n]/g, "").substring(2)), t = n.shift();
            }return [["code_block", r.join("\n")]];
          }, horizRule: function horizRule(t, n) {
            var r = t.match(/^(?:([\s\S]*?)\n)?[ \t]*([-_*])(?:[ \t]*\2){2,}[ \t]*(?:\n([\s\S]*))?$/);if (!r) return void 0;var i = [["hr"]];if (r[1]) {
              var s = p(r[1], "", t.lineNumber);i.unshift.apply(i, this.toTree(s, []));
            }return (r[3] && n.unshift(p(r[3], t.trailing, t.lineNumber + 1)), i);
          }, lists: (function () {
            function e(e) {
              return new RegExp("(?:^(" + a + "{0," + e + "} {0,3})(" + s + ")\\s+)|" + "(^" + a + "{0," + (e - 1) + "}[ ]{0,4})");
            }function t(e) {
          ...
          })(), blockquote: function blockquote(t, n) {
            if (!t.match(/^>/m)) return void 0;var r = [];if (">" !== t[0]) {
              for (var i = t.split(/\n/), s = [], o = t.lineNumber; i.length && ">" !== i[0][0];) s.push(i.shift()), o++;var u = p(s.join("\n"), "\n", t.lineNumber);r.push.apply(r, this.processBlock(u, [])), t = p(i.join("\n"), t.trailing, o);
            }for (; n.length && ">" === n[0][0];) {
              var a = n.shift();t = p(t + t.trailing + a, a.trailing, t.lineNumber);
            }var f = t.replace(/^> ?/gm, ""),
                l = (this.tree, this.toTree(f, ["blockquote"])),
                c = d(l);return (c && c.references && (delete c.references, g(c) && l.splice(1, 1)), r.push(l), r);
          }, referenceDefn: function referenceDefn(t, n) {
            var r = /^\s*\[(.*?)\]:\s*(\S+)(?:\s+(?:(['"])(.*?)\3|\((.*?)\)))?\n?/;if (!t.match(r)) return void 0;d(this.tree) || this.tree.splice(1, 0, {});var i = d(this.tree);void 0 === i.references && (i.references = {});var s = this.loop_re_over_block(r, t, function (e) {
              e[2] && "<" === e[2][0] && ">" === e[2][e[2].length - 1] && (e[2] = e[2].substring(1, e[2].length - 1));var t = i.references[e[1].toLowerCase()] = { href: e[2] };void 0 !== e[4] ? t.title = e[4] : void 0 !== e[5] && (t.title = e[5]);
            });return (s.length && n.unshift(p(s, t.trailing)), []);
          }, para: function para(t) {
            return [["para"].concat(this.processInline(t))];
          } }, inline: { __oneElement__: function __oneElement__(t, n, r) {
            var i, s;n = n || this.dialect.inline.__patterns__;var o = new RegExp("([\\s\\S]*?)(" + (n.source || n) + ")");if ((i = o.exec(t), !i)) return [t.length, t];if (i[1]) return [i[1].length, i[1]];var s;return (i[2] in this.dialect.inline && (s = this.dialect.inline[i[2]].call(this, t.substr(i.index), i, r || [])), s = s || [i[2].length, i[2]]);
          }, __call__: function __call__(t, n) {
            function r(e) {
              "string" == typeof e && "string" == typeof s[s.length - 1] ? s[s.length - 1] += e : s.push(e);
            }for (var i, s = []; t.length > 0;) i = this.dialect.inline.__oneElement__.call(this, t, n, s), t = t.substr(i.shift()), m(i, r);return s;
          }, "]": function _() {}, "}": function _() {}, __escape__: /^\\[\\`\*_{}\[\]()#\+.!\-]/, "\\": function _(t) {
            return this.dialect.inline.__escape__.exec(t) ? [2, t.charAt(1)] : [1, "\\"];
          }, "![": function _(t) {
            var n = t.match(/^!\[(.*?)\][ \t]*\([ \t]*([^")]*?)(?:[ \t]+(["'])(.*?)\3)?[ \t]*\)/);if (n) {
              n[2] && "<" === n[2][0] && ">" === n[2][n[2].length - 1] && (n[2] = n[2].substring(1, n[2].length - 1)), n[2] = this.dialect.inline.__call__.call(this, n[2], /\\/)[0];var r = { alt: n[1], href: n[2] || "" };return (void 0 !== n[4] && (r.title = n[4]), [n[0].length, ["img", r]]);
            }return (n = t.match(/^!\[(.*?)\][ \t]*\[(.*?)\]/), n ? [n[0].length, ["img_ref", { alt: n[1], ref: n[2].toLowerCase(), original: n[0] }]] : [2, "!["]);
          }, "[": function S(e) {
            var t = String(e),
                n = y.call(this, e.substr(1), "]");if (!n) return [1, "["];var S,
                r,
                i = 1 + n[0],
                s = n[1];e = e.substr(i);var o = e.match(/^\s*\([ \t]*([^"']*)(?:[ \t]+(["'])(.*?)\2)?[ \t]*\)/);if (o) {
              var u = o[1];if ((i += o[0].length, u && "<" === u[0] && ">" === u[u.length - 1] && (u = u.substring(1, u.length - 1)), !o[3])) for (var a = 1, f = 0; f < u.length; f++) switch (u[f]) {case "(":
                  a++;break;case ")":
                  0 === --a && (i -= u.length - f, u = u.substring(0, f));}return (u = this.dialect.inline.__call__.call(this, u, /\\/)[0], r = { href: u || "" }, void 0 !== o[3] && (r.title = o[3]), S = ["link", r].concat(s), [i, S]);
            }return (o = e.match(/^\s*\[(.*?)\]/), o ? (i += o[0].length, r = { ref: (o[1] || String(s)).toLowerCase(), original: t.substr(0, i) }, S = ["link_ref", r].concat(s), [i, S]) : 1 === s.length && "string" == typeof s[0] ? (r = { ref: s[0].toLowerCase(), original: t.substr(0, i) }, S = ["link_ref", r, s[0]], [i, S]) : [1, "["]);
          }, "<": function _(t) {
            var n;return null !== (n = t.match(/^<(?:((https?|ftp|mailto):[^>]+)|(.*?@.*?\.[a-zA-Z]+))>/)) ? n[3] ? [n[0].length, ["link", { href: "mailto:" + n[3] }, n[3]]] : "mailto" === n[2] ? [n[0].length, ["link", { href: n[1] }, n[1].substr("mailto:".length)]] : [n[0].length, ["link", { href: n[1] }, n[1]]] : [1, "<"];
          }, "`": function _(t) {
            var n = t.match(/(`+)(([\s\S]*?)\1)/);return n && n[2] ? [n[1].length + n[2].length, ["inlinecode", n[3]]] : [1, "`"];
          }, "  \n": function _() {
            return [3, ["linebreak"]];
          } } };b.inline["**"] = a("strong", "**"), b.inline.__ = a("strong", "__"), b.inline["*"] = a("em", "*"), b.inline._ = a("em", "_"), h.dialects.Gruber = b, h.buildBlockOrder(h.dialects.Gruber.block), h.buildInlinePatterns(h.dialects.Gruber.inline);var w = v.subclassDialect(b),
          d = l.extract_attr,
      ...
      }, w.block.table = function x(e) {
        var t,
            n,
            r = function u(e, t) {
          t = t || "\\s", t.match(/^[\\|\[\]{}?*.+^$]$/) && (t = "\\" + t);for (var n, u = [], r = new RegExp("^((?:\\\\.|[^\\\\" + t + "])*)" + t + "(.*)"); n = e.match(r);) u.push(n[1]), e = n[2];return (u.push(e), u);
        },
            i = /^ {0,3}\|(.+)\n {0,3}\|\s*([\-:]+[\-| :]*)\n((?:\s*\|.*(?:\n|$))*)(?=\n|$)/,
            s = /^ {0,3}(\S(?:\\.|[^\\|])*\|.*)\n {0,3}([\-:]+\s*\|[\-| :]*)\n((?:(?:\\.|[^\\|])*\|.*(?:\n|$))*)(?=\n|$)/;if (n = e.match(i)) n[3] = n[3].replace(/^\s*\|/gm, "");else if (!(n = e.match(s))) return void 0;var x = ["table", ["thead", ["tr"]], ["tbody"]];n[2] = n[2].replace(/\|\s*$/, "").split("|");var o = [];for (m(n[2], function (e) {
          e.match(/^\s*-+:\s*$/) ? o.push({ align: "right" }) : e.match(/^\s*:-+\s*$/) ? o.push({ align: "left" }) : e.match(/^\s*:-+:\s*$/) ? o.push({ align: "center" }) : o.push({});
        }), n[1] = r(n[1].replace(/\|\s*$/, ""), "|"), t = 0; t < n[1].length; t++) x[1][1].push(["th", o[t] || {}].concat(this.processInline(n[1][t].trim())));return (m(n[3].replace(/\|\s*$/gm, "").split("\n"), function (e) {
          var n = ["tr"];for (e = r(e, "|"), t = 0; t < e.length; t++) n.push(["td", o[t] || {}].concat(this.processInline(e[t].trim())));x[2].push(n);
        }, this), [x]);
      }, w.inline["{:"] = function (e, t, n) {
        if (!n.length) return [2, "{:"];var r = n[n.length - 1];if ("string" == typeof r) return [2, "{:"];var i = e.match(/^\{:\s*((?:\\\}|[^\}])*)\s*\}/);if (!i) return [2, "{:"];var s = this.dialect.processMetaHash(i[1]),
            o = d(r);o || (o = {}, r.splice(1, 0, o));for (var u in s) o[u] = s[u];return [i[0].length, ""];
    ...
    (function (e, t) {
      if (typeof exports == "object") module.exports = t(require("components/nova-markdown/main"));else if (typeof define == "function" && define.amd) define("components/nova-markdown-editor/main", ["components/nova-markdown/main"], t);else {
        var n = "Nova.Components.NovaMarkdownEditor",
            r = n.split("."),
            i = e;for (var s = 0; s < r.length - 1; s++) i[r[s]] === undefined && (i[r[s]] = {}), i = i[r[s]];i[r[r.length - 1]] = t(e._4);
      }
    })(this, function (e) {
      function t(t) {
