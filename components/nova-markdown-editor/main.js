...
(function () {
  var e, t;(function () {
    var e, t, n;(function () {
      function t(e) {
        this.tokens = [], this.tokens.links = {}, this.options = e || l.defaults, this.rules = c.normal, this.options.gfm && (this.options.tables ? this.rules = c.tables : this.rules = c.gfm);
      }function n(e, t) {
        this.options = t || l.defaults, this.links = e, this.rules = h.normal, this.renderer = this.options.renderer || new r(), this.renderer.options = this.options;if (!this.links) throw new Error("Tokens array requires a `links` property.");this.options.gfm ? this.options.breaks ? this.rules = h.breaks : this.rules = h.gfm : this.options.pedantic && (this.rules = h.pedantic);
      }function r(e) {
        this.options = e || {};
      }function i(e) {
        ...
        if (r || typeof n == "function") {
          r || (r = n, n = null), n = f({}, l.defaults, n || {});var o = n.highlight,
              u,
              a,
              c = 0;try {
            u = t.lex(e, n);
            ...
            return r(h);
          }a = u.length;var p = function p(t) {
            if (t) return (n.highlight = o, r(t));var s;try {
              s = i.parse(u, n);
            } catch (a) {
              t = a;
            }return (n.highlight = o, t ? r(t) : r(null, s));
          };if (!o || o.length < 3) return p();delete n.highlight;if (!a) return p();for (; c < u.length; c++) (function (e) {
            return e.type !== "code" ? --a || p() : o(e.text, e.lang, function (t, n) {
              if (t) return p(t);if (n == null || n === e.text) return --a || p();e.text = n, e.escaped = !0, --a || p();
            });
          })(u[c]);return;
        }try {
          return (n && (n = f({}, l.defaults, n)), i.parse(t.lex(e, n), n));
        } catch (h) {
          h.message += "\nPlease report this to https://github.com/chjj/marked.";if ((n || l.defaults).silent) return "<p>An error occured:</p><pre>" + s(h.message + "", !0) + "</pre>";throw h;
        }
      }var c = { newline: /^\n+/, code: /^( {4}[^\n]+\n*)+/, fences: a, hr: /^( *[-*_]){3,} *(?:\n+|$)/, heading: /^ *(#{1,6}) *([^\n]+?) *#* *(?:\n+|$)/, nptable: a, lheading: /^([^\n]+)\n *(=|-){2,} *(?:\n+|$)/, blockquote: /^( *>[^\n]+(\n(?!def)[^\n]+)*\n*)+/, list: /^( *)(bull) [\s\S]+?(?:hr|def|\n{2,}(?! )(?!\1bull )\n*|\s*$)/, html: /^ *(?:comment *(?:\n|\s*$)|closed *(?:\n{2,}|\s*$)|closing *(?:\n{2,}|\s*$))/, def: /^ *\[([^\]]+)\]: *<?([^\s>]+)>?(?: +["(]([^\n]+)[")])? *(?:\n+|$)/, table: a, paragraph: /^((?:[^\n]+\n?(?!hr|heading|lheading|blockquote|tag|def))+)\n*/, text: /^[^\n]+/ };c.bullet = /(?:[*+-]|\d+\.)/, c.item = /^( *)(bull) [^\n]*(?:\n(?!\1bull )[^\n]*)*/, c.item = u(c.item, "gm")(/bull/g, c.bullet)(), c.list = u(c.list)(/bull/g, c.bullet)("hr", "\\n+(?=\\1?(?:[-*_] *){3,}(?:\\n+|$))")("def", "\\n+(?=" + c.def.source + ")")(), c.blockquote = u(c.blockquote)("def", c.def)(), c._tag = "(?!(?:a|em|strong|small|s|cite|q|dfn|abbr|data|time|code|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo|span|br|wbr|ins|del|img)\\b)\\w+(?!:/|[^\\w\\s@]*@)\\b", c.html = u(c.html)("comment", /<!--[\s\S]*?-->/)("closed", /<(tag)[\s\S]+?<\/\1>/)("closing", /<tag(?:"[^"]*"|'[^']*'|[^'">])*?>/)(/tag/g, c._tag)(), c.paragraph = u(c.paragraph)("hr", c.hr)("heading", c.heading)("lheading", c.lheading)("blockquote", c.blockquote)("tag", "<" + c._tag)("def", c.def)(), c.normal = f({}, c), c.gfm = f({}, c.normal, { fences: /^ *(`{3,}|~{3,})[ \.]*(\S+)? *\n([\s\S]*?)\s*\1 *(?:\n+|$)/, paragraph: /^/, heading: /^ *(#{1,6}) +([^\n]+?) *#* *(?:\n+|$)/ }), c.gfm.paragraph = u(c.paragraph)("(?!", "(?!" + c.gfm.fences.source.replace("\\1", "\\2") + "|" + c.list.source.replace("\\1", "\\3") + "|")(), c.tables = f({}, c.gfm, { nptable: /^ *(\S.*\|.*)\n *([-:]+ *\|[-| :]*)\n((?:.*\|.*(?:\n|$))*)\n*/, table: /^ *\|(.+)\n *\|( *[-:]+[-| :]*)\n((?: *\|.*(?:\n|$))*)\n*/ }), t.rules = c, t.lex = function (e, n) {
        ...
        var e = e.replace(/^ +$/gm, ""),
          ...
          if (s = this.rules.newline.exec(e)) e = e.substring(s[0].length), s[0].length > 1 && this.tokens.push({ type: "space" });if (s = this.rules.code.exec(e)) {
            ...
            e = e.substring(s[0].length), a = { type: "table", header: s[1].replace(/^ *| *\| *$/g, "").split(/ *\| */), align: s[2].replace(/^ *|\| *$/g, "").split(/ *\| */), cells: s[3].replace(/\n$/, "").split("\n") };for (l = 0; l < a.align.length; l++) /^ *-+: *$/.test(a.align[l]) ? a.align[l] = "right" : /^ *:-+: *$/.test(a.align[l]) ? a.align[l] = "center" : /^ *:-+ *$/.test(a.align[l]) ? a.align[l] = "left" : a.align[l] = null;for (l = 0; l < a.cells.length; l++) a.cells[l] = a.cells[l].split(/ *\| */);this.tokens.push(a);continue;
            ...
            e = e.substring(s[0].length), o = s[2], this.tokens.push({ type: "list_start", ordered: o.length > 1 }), s = s[0].match(this.rules.item), r = !1, h = s.length, l = 0;for (; l < h; l++) a = s[l], f = a.length, a = a.replace(/^ *([*+-]|\d+\.) +/, ""), ~a.indexOf("\n ") && (f -= a.length, a = this.options.pedantic ? a.replace(/^ {1,4}/gm, "") : a.replace(new RegExp("^ {1," + f + "}", "gm"), "")), this.options.smartLists && l !== h - 1 && (u = c.bullet.exec(s[l + 1])[0], o !== u && !(o.length > 1 && u.length > 1) && (e = s.slice(l + 1).join("\n") + e, l = h - 1)), i = r || /\n\n(?!\s*$)/.test(a), l !== h - 1 && (r = a.charAt(a.length - 1) === "\n", i || (i = r)), this.tokens.push({ type: i ? "loose_item_start" : "list_item_start" }), this.token(a, !1, n), this.tokens.push({ type: "list_item_end" });this.tokens.push({ type: "list_end" });continue;
            ...
            e = e.substring(s[0].length), a = { type: "table", header: s[1].replace(/^ *| *\| *$/g, "").split(/ *\| */), align: s[2].replace(/^ *|\| *$/g, "").split(/ *\| */), cells: s[3].replace(/(?: *\| *)?\n$/, "").split("\n") };for (l = 0; l < a.align.length; l++) /^ *-+: *$/.test(a.align[l]) ? a.align[l] = "right" : /^ *:-+: *$/.test(a.align[l]) ? a.align[l] = "center" : /^ *:-+ *$/.test(a.align[l]) ? a.align[l] = "left" : a.align[l] = null;for (l = 0; l < a.cells.length; l++) a.cells[l] = a.cells[l].replace(/^ *\| *| *\| *$/g, "").split(/ *\| */);this.tokens.push(a);continue;
        ...
        var t = "",
          ...
          if (o = this.rules.escape.exec(e)) {
            ...
            e = e.substring(o[0].length), o[2] === "@" ? (r = o[1].charAt(6) === ":" ? this.mangle(o[1].substring(7)) : this.mangle(o[1]), i = this.mangle("mailto:") + r) : (r = s(o[1]), i = r), t += this.renderer.link(i, null, r);continue;
          }if (!this.inLink && (o = this.rules.url.exec(e))) {
            e = e.substring(o[0].length), r = s(o[1]), i = r, t += this.renderer.link(i, null, r);continue;
          }if (o = this.rules.tag.exec(e)) {
            !this.inLink && /^<a /i.test(o[0]) ? this.inLink = !0 : this.inLink && /^<\/a>/i.test(o[0]) && (this.inLink = !1), e = e.substring(o[0].length), t += this.options.sanitize ? this.options.sanitizer ? this.options.sanitizer(o[0]) : s(o[0]) : o[0];continue;
            ...
            e = e.substring(o[0].length), t += this.renderer.codespan(s(o[2], !0));continue;
            ...
            e = e.substring(o[0].length), t += this.renderer.text(s(this.smartypants(o[0])));continue;
          }if (e) throw new Error("Infinite loop on byte: " + e.charCodeAt(0));
        }return t;
        ...
        var r = new i(t, n);return r.parse(e);
      }, i.prototype.parse = function (e) {
        this.inline = new n(e.links, this.options, this.renderer), this.tokens = e.reverse();var t = "";while (this.next()) t += this.tok();return t;
        ...
        return (f(l.defaults, e), l);
      }, l.defaults = { gfm: !0, tables: !0, breaks: !1, pedantic: !1, sanitize: !1, sanitizer: null, mangle: !0, smartLists: !1, silent: !1, highlight: null, langPrefix: "lang-", smartypants: !1, headerPrefix: "", renderer: new r(), xhtml: !1 }, l.Parser = i, l.parser = i.parse, l.Renderer = r, l.Lexer = t, l.lexer = t.lex, l.InlineLexer = n, l.inlineLexer = n.output, l.parse = l, typeof module != "undefined" && typeof exports == "object" ? module.exports = l : e = (function () {
        return l;
      })();
    }).call((function () {
      ...
      function t(e) {
        ...
        function o(e, t) {
          ...
          var n = !0;e: while (n) {
            var i = e,
                s = t;n = !1;if (r(i.eR, s)) {
              for (; i.endsParent && i.parent;) i = i.parent;return i;
            }if (i.eW) {
              e = i.parent, t = s, n = !0;continue e;
            }return void 0;
          }
        }function a(e, t) {
          return !i && r(t.iR, e);
      ...
      (function (r, i) {
        ...
        function n(n) {
          return ({ "components/nova-markdown/marked": e, "components/nova-markdown/highlight.pack": t })[n];
            ...
            this.on("_contentChanged", this.contentChanged);var t = this.querySelector("script");this.content = t ? t.innerHTML : this.innerHTML;
          }, contentChanged: function contentChanged() {
            this.innerHTML = marked(this.content), this.hljs && hljs.configure(hljs);var t = Array.prototype.slice.call(this.querySelectorAll("pre code"));t.forEach(function (e, t) {
