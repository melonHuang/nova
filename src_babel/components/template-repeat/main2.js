NovaExports.exports={"stylesheet":"<style>\n        :host {display:none;}\n    </style>","template":"<template>\n    </template>"};
        ...
        let TemplateRepeat = NovaExports({
            ...
            itemsObserver: function(ev, oldVal, newVal, path, noNeedToRender) {
                if(path != 'items') return;


                console.log('regenerated repeat html');
                // 由最后一行主动触发时，无需重新渲染
                if(noNeedToRender) {return;}
                if(!newVal || newVal.constructor != Array) {return;}

                // 进行diff，若无改变，则返回
                let valString = JSON.stringify(newVal)
                if(this._lastVal == valString) {return;}
            ...
            appendItem: function(index) {
                ...
                let tmpl = this.repeatTemplate.replace(/{{items\[i\]:index}}/g, function() {
                    return index;
                });
                ...
                // 插入到父元素
                let nodes = Array.prototype.slice.call(wrap.childNodes);
            ...
            removeItem: function(index) {
                let self = this;
                let nodes = this.itemNodes.splice(index, 1)[0];
                nodes.forEach(function(node) {
            ...
            */
            ...
            compileNode: function(node) {
                ...
                if(node.nodeType == Node.TEXT_NODE) {
                    let cont = node.textContent.replace(/{{item(\..+|:index)?}}/g, function(match, p) {
                        return '{{items[i]' + (p || '') + '}}';
                    });
                    ...
                    Array.prototype.slice.call(node.attributes || []).forEach(function(attr) {
                        if(attr.constructor == Attr) {
                            let match = attr.value.match(/^{{item(\..+|:index)?}}$/);
                            if(match) {
